...
# Si hay evidencia significativa para concluir que las varianzas de las muestras son diferentes, el p-valor debe ser chico,
# esto nos confirma que es posible reducir la cantidad de variables para poder simplificar la información

KMO(cor(datos))                       # Índice KMO
# Evalua si los datos tienen suficiente variabilidad y estructura para justificar la reducción de dimensionalidad,
# donde valores más cercanos a 1 indican una mayor adecuación para realizar ACP.
...
# Donde vemos a la opción más adecuada de CP's a elegir, ya que explicaría la mayor
# cantidad de información con la menor cantidad de componentes necesarios.
...
# Las relaciones entre las variables originales y las componentes principales resultantes del análisis.
...
cor(x = mat_dist, cophenetic(hc_euclidea_average)) # Coeficiente de Correlacion
# Cuanto más cercano es el valor a 1, mejor refleja el dendrograma la verdadera similitud entre las
# observaciones. Valores superiores a 0.75 suelen considerarse como buenos.
...
# Un valor de silueta cercano a 1 indica que el punto está bien clasificado en su clúster y está lejos de otros clústeres, lo que sugiere una buena separación de los clústeres.
# Un valor de silueta cercano a 0 indica que el punto está cerca del límite entre dos clústeres o que no está claramente asignado a ningún clúster en particular.
# Un valor de silueta cercano a -1 indica que el punto puede haber sido asignado incorrectamente a su clúster y está más cerca de puntos en otro clúster.
